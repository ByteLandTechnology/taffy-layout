...
- `flexGrow` のデフォルトは `0` です。基準サイズは `flexBasis`、`size`、コンテンツから決まり、空きスペースによる伸長は行いません。
- すべてのアイテムが `flexGrow: 1` なら、min/max 制約がない限り残りのスペースを均等に追加します。基準サイズが異なる場合、最終的な幅が等しくなるとは限りません。
