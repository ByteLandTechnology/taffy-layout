...
| **`Absolute`** | 要素は**フローから除外されます**。直接の親の配置領域を基準に配置され、通常は親の padding box を使います。Grid では配置指定によってグリッド領域が基準になることがあります。 |

この API の `position` は `Relative` と `Absolute` のみです。ブラウザの `static` の祖先をたどる規則や、`fixed` / `sticky` の動作は提供しません。計算結果の `Layout.x` / `Layout.y` は常に直接の親を基準とする物理座標です。
