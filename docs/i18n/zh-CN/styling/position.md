...
| **`Absolute`** | 元素**从流中移除**。由直接父布局容器定位；在 Grid 中还可通过网格线限定定位区域。       |

绑定没有 CSS 的 `position: static`：默认的 `Relative` 父节点也能作为绝对定位子节点的参照。普通绝对定位使用父 padding box，不会跳过默认定位的父节点去寻找更高层祖先。
